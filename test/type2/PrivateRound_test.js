...
describe('PrivateRoundContract', () => {
    let contract;
    let token;

    async function getContracts(){
        /*Can be used for Advisors, Reserved and PlayerRewards*/
        let contractTemp = await ethers.getContractFactory('PrivateRoundContract');
    ...
    describe('addWhiteList', () => {
        it("Should add to Whitelist", async function() {
            ...
            expect(await token.balanceOf(acc1.address)).to.equal(1000000000000000000n)
            expect(await contract.ownedBFG(acc1.address)).to.equal(10000000000000000000n)
            expect(await contract.lockedBFG(acc1.address)).to.equal(9000000000000000000n)
            ...
            expect(await contract.lastUnlockTime(acc1.address)).to.equal(1680795000)
        })
    ...
    describe('withdraw', () => {
        ...
        it("1 day have passed after 6 month cliff", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
            ...
            expect(await token.balanceOf(owner.address)).to.equal(1025000000000000000n)
        })

        it("10 days have passed after 6 month cliff", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
            ...
            expect(await token.balanceOf(owner.address)).to.equal(1250000000000000000n)
        })

        it("100 days have passed after 6 month cliff", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
            ...
            expect(await token.balanceOf(owner.address)).to.equal(3475000000000000000n)
            expect(await contract.lockedBFG(owner.address)).to.equal(6525000000000000000n)
        })
        ...
        it("Vesting has ended", async function() {
            ...
            await contract.withdraw(1759505738, 1696347000)
            
            expect(await token.balanceOf(owner.address)).to.equal(10000000000000000000n)
            expect(await contract.ownedBFG(owner.address)).to.equal(10000000000000000000n)
