...
describe('AdvisorsContract', () => {
    ...
    describe('addWhiteList', () => {
        it("Should add to Whitelist", async function() {
            ...
            expect(await contract.lastUnlockTime(acc1.address)).to.equal(1696347000)
        })
    ...
    describe('withdraw', () => {
        ...
        it("Revert if not 12 months have passed", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
        ...
        it("1 day have passed after 12 month cliff", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
            ...
            expect(await token.balanceOf(owner.address)).to.equal(13900000000000000n)
        })

        it("10 days have passed after 12 month cliff", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
            ...
            expect(await token.balanceOf(owner.address)).to.equal(139000000000000000n)
        })

        it("100 days have passed after 12 month cliff", async function() {
            const { contract, owner, acc1, acc2, token } = await loadFixture(getContracts);
            ...
            expect(await token.balanceOf(owner.address)).to.equal(1376100000000000000n)
            expect(await contract.lockedBFG(owner.address)).to.equal(8623900000000000000n)
        })
